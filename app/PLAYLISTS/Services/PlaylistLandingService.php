...
final class PlaylistLandingService
{
    ...
    public function generateSlug(
        ...
        return $this->playlistRepo->generateUniqueSlug(
            $slug,
            $excludePlaylistId
        );
    }
     ...
     */
    public function buildLandingViewModel(
        array $row,
        string $origin
    ): array {
        ...
        $heroImage = $this->normalizeImageUrl(
            (string)($row['hero_image_url'] ?? ''),
            $origin
        );
         ...
         * Legacy watch gating.
         *
         * This remains intentionally unchanged during the namespace move.
         * It will be replaced with the REX boundary during the cleanup pass.
         */
        $watchInstanceId =
            (int)($row['watch_playlist_instance_id'] ?? 0);

        $playlistId =
            (int)($row['playlist_id'] ?? 0);
    ...
    public function truncatePlainText(
        string $htmlOrText,
        int $maxChars = 160
    ): string {
        $plain = trim(
            html_entity_decode(
                strip_tags($htmlOrText),
    ...
    private function normalizeImageUrl(
        string $url,
        string $origin
    ): string {
        $trimmed = trim($url);
