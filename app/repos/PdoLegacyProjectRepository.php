...
final class PdoLegacyProjectRepository
{
    ...
    public function listWithAssetCounts(array $filters = []): array
    {
        ...
        if ($query !== '') {
            $where[] = '(p.title LIKE :q_title OR p.slug LIKE :q_slug OR p.client_name LIKE :q_client_name)';
            $params[':q_title'] = '%' . $query . '%';
            $params[':q_slug'] = '%' . $query . '%';
            $params[':q_client_name'] = '%' . $query . '%';
        }
        ...
        $sql = "
            ...
            LEFT JOIN legacy_project_links pl ON pl.project_id = p.id
        ";
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM legacy_projects WHERE id = :id LIMIT 1');
        $stmt->execute([':id' => $id]);
    ...
    {
        $stmt = $this->pdo->prepare('DELETE FROM legacy_projects WHERE id = :id');
        $stmt->execute([':id' => $id]);
    }
}
