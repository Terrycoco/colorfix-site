...
final class AssetCreatorRunService
{
    ...
    private function createCompositePin(array $job, array $pair, int $pairOrder): array
    {
        ...
        $clientId = $this->clientIdForPair($before, $after);
    ...
    private function clientIdForPair(array $before, array $after): ?int
    {
        $beforeId = (int)($before['client_id'] ?? 0);
        $afterId = (int)($after['client_id'] ?? 0);
        if ($beforeId > 0 && $afterId > 0 && $beforeId !== $afterId) {
            return null;
        }
        return $afterId > 0 ? $afterId : ($beforeId > 0 ? $beforeId : null);
    }
    ...
    private function drawBadge(\GdImage $canvas, string $label, int $x, int $y): void
    {
        ...
        if ($font) {
            ...
            foreach ([[-2, 0], [-1, -1], [-1, 0], [-1, 1], [0, -1], [0, 0], [0, 1], [1, -1], [1, 0], [1, 1], [2, 0]] as [$dx, $dy]) {
                imagettftext($canvas, $fontSize, 0, $textX + $dx, $textY + $dy, $white, $font, $label);
            }
            return;
        }
    ...
    private function drawPinLogo(\GdImage $canvas): void
    {
        ...
        $dstW = self::PIN_LOGO_WIDTH;
        $dstH = max(1, (int)round($srcH * ($dstW / max(1, $srcW))));
        $dstX = self::PIN_WIDTH - self::PIN_LOGO_MARGIN - $dstW;
        $dstY = self::PIN_HEIGHT - self::PIN_LOGO_MARGIN - $dstH;
    ...
    private function pinLogoNeedsBadge(\GdImage $canvas, int $x, int $y, int $width, int $height): bool
    {
        $luminance = [];
        $step = 8;
        for ($sampleY = max(0, $y); $sampleY < min(self::PIN_HEIGHT, $y + $height); $sampleY += $step) {
            for ($sampleX = max(0, $x); $sampleX < min(self::PIN_WIDTH, $x + $width); $sampleX += $step) {
                $rgb = imagecolorat($canvas, $sampleX, $sampleY);
                $r = ($rgb >> 16) & 0xFF;
                $g = ($rgb >> 8) & 0xFF;
                $b = $rgb & 0xFF;
        ...
        $avg = array_sum($luminance) / count($luminance);
        $variance = 0.0;
        foreach ($luminance as $value) {
            $variance += ($value - $avg) ** 2;
        }
        $contrast = sqrt($variance / count($luminance));

        return $avg < 160 || $contrast > 42;
    }

    private function findPinLogoPath(): string
    {
        $candidates = [
            $this->rootDir . '/brand/' . self::PIN_LOGO_FILE,
            $this->rootDir . '/public/brand/' . self::PIN_LOGO_FILE,
            dirname($this->rootDir) . '/public/brand/' . self::PIN_LOGO_FILE,
        ];
