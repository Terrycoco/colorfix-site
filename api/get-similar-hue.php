...
// read ids from GET or JSON POST
$ids = [];
if (isset($_GET['ids'])) {
  $raw = $_GET['ids'];
  if (is_array($raw)) {
...
// sanitize -> ints > 0, dedupe, preserve order
$seen = [];
$clean = [];
foreach ($ids as $v) {
  $n = (int)$v;
...
// build optional brand clause for sv.brand
$brandClause = '';
$brandParams = [];
if (!empty($brandCodes)) {
...
// NOTE: use ONLY positional placeholders.
// If tol>0, we add ONE more "?" and push $tol into params after the ids.
$joinCond = $tol > 0
  ? "ABS(MOD((sv.hcl_h - bc.hue_r + 540), 360) - 180) <= ?"
  : "MOD(ROUND(sv.hcl_h), 360) = bc.hue_r";

$sql = "
  SELECT
    sv.*,
    CONCAT('Colors with the same hue as ', bc.header_name) AS group_header,
...
// Build params in LEFT-TO-RIGHT order of the SQL placeholders:
// 1) seed ids (IN ($placeholders))
// 2) tol (IF used)
// 3) brand codes (IF any)
$params = $ids;
if ($tol > 0) { $params[] = $tol; }
