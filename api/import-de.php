...
function isStain(string $collection, string $name, string $descr, string $url): int {
  $hay = strtolower($collection.' '.$name.' '.$descr.' '.$url);
  return (int)(strpos($hay, 'stain') !== false);
}
...
$csv = __DIR__.'/data/de-colors.csv';
$log = __DIR__.'/import-de.log';
logf($log, "=== DE import started ===");
...
ON DUPLICATE KEY UPDATE
  ...
  lrv         = COALESCE(VALUES(lrv), lrv),
  interior    = COALESCE(VALUES(interior), interior),
  exterior    = COALESCE(VALUES(exterior), exterior),
  is_stain    = COALESCE(VALUES(is_stain), is_stain)
SQL;
  ...
  while (($row = fgetcsv($fh)) !== false) {
    $total++;
    if (count($row) < 8) { $skipped++; logf($log, "skip: too few cols (#$total)"); continue; }

    [$code,$hex,$collection,$name,$descr,$lrv_raw,$usage,$url] = array_map('trim',$row);
    if ($hex === '' || $name === '' || $url === '') {
    ...
    $lrv = is_numeric($lrv_raw) ? (float)$lrv_raw : null;
    $nameTC = titleCase($name);
    $codeUC = strtoupper($code);

    // interior/exterior flags from $usage
    $usage_lc = strtolower((string)$usage);
    $interior = (int)((strpos($usage_lc,'interior') !== false) || (strpos($usage_lc,'exterior') === false));
    $exterior = (int)(strpos($usage_lc,'exterior') !== false);
    if (strpos($usage_lc,'interior use only') !== false) { $interior=1; $exterior=0; }
    if (strpos($usage_lc,'exterior use only') !== false) { $interior=0; $exterior=1; }

    $is_stain = isStain((string)$collection, $nameTC, (string)$descr, (string)$url);
    ...
    try {
      ...
      $rc = $ins->rowCount();
      // ON DUPLICATE KEY UPDATE returns 1 on insert, 2 on update (sometimes 0 if no change)
  ...
  // Trigger HCL/cluster only for newly inserted hexes
  if ($inserted > 0) {
    triggerRecalcHexes($newHexes);
    logf($log, "Triggered recalc for ~".count(array_unique($newHexes))." new hexes");
  }

  echo "✅ Import complete. total=$total inserted=$inserted updated=$updated skipped=$skipped\nSee log: import-de.log\n";

} catch (Throwable $e) {
  if ($pdo->inTransaction()) $pdo->rollBack();
  logf($log, "❌ Fatal: ".$e->getMessage());
  http_response_code(500);
  echo "❌ Import failed. See import-de.log\n";
}
