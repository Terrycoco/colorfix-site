...
function normalize_activity_row(array $row): array
{
    ...
    return [
        ...
        'admin_read_at' => isset($row['admin_read_at']) ? (string)$row['admin_read_at'] : '',
        'needs_attention' => (string)($row['activity_type'] ?? '') === 'site_note_received' && empty($row['admin_read_at']),
        ...
        'email' => !empty($row['related_client_email_id']) ? [
            ...
            'from_email' => (string)($row['email_from_email'] ?? ''),
