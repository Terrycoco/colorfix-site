...
export default function PlaylistSlideEditor({
  ...
  const availablePalettes = useMemo(() => {
    ...
    return (Array.isArray(savedPalettes) ? savedPalettes : [])
      .filter((palette) => {
        if (currentProjectId <= 0) return true;
        return Number(palette?.project_id || 0) === currentProjectId;
      })
      .slice()
      .sort((a, b) => {
        const aLabel = String(
          ...
          a?.nickname ||
          a?.label ||
          `Palette #${a?.saved_palette_id || a?.id || ""}`
        ).trim();
        ...
        const bLabel = String(
          ...
          b?.nickname ||
          b?.label ||
          `Palette #${b?.saved_palette_id || b?.id || ""}`
        ).trim();
  ...
  }, [savedPalettes, projectId]);

  const slidePaletteHash = String(
    item?.palette_hash ||
    ""
  ).trim();

  const selectedProjectPalette = useMemo(
    () =>
      availablePalettes.find(
        (palette) =>
          String(
            ...
            ""
          ).trim() === slidePaletteHash
      ) || null,
    [availablePalettes, slidePaletteHash]
  );

  function setProjectPalette(value) {
    ...
    <div onBlurCapture={() => onCommit?.()}>
      ...
      <AdminPanel title="Photo / Palette" compact>
        ...
        <AdminStack gap="sm">
          ...
          <select
            className="admin-field__control"
            value={slidePaletteHash}
            onChange={(event) =>
              setProjectPalette(event.target.value)
            }
          >
            <option value="">
