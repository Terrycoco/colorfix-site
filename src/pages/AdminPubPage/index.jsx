...
const PIN_TYPES = [
  ...
  { value: "before_after_video", label: "Before / After Video" },
  { value: "idea", label: "Idea" },
  { value: "idea_palette", label: "Idea + Palette" },
  { value: "youtube_teaser", label: "YouTube Teaser" },
];
...
export default function AdminPubPage() {
  ...
  async function analyzePinterest() {
    ...
    try {
      ...
      const nextProposals = Array.isArray(
        nextAnalysis?.pinterest?.asset_proposals
      )
        ? nextAnalysis.pinterest.asset_proposals.map(normalizeProposal)
        : [];
  ...
  function updateProposal(proposalKey, changes) {
    setProposals((current) =>
      current.map((proposal) =>
        proposal.proposal_key === proposalKey
          ? { ...proposal, ...changes }
          : proposal
      )
    );
  ...
  function removeProposal(proposalKey) {
    setProposals((current) =>
      current.filter(
        (proposal) => proposal.proposal_key !== proposalKey
      )
    );
  }

  async function previewBeforeAfterVideo(proposal) {
    if (!proposal?.before?.image_url) {
      setError("Before image required.");
      return;
    }

    if (!proposal?.after?.image_url) {
      setError("After image required.");
      return;
    }
  ...
  const visibleProposals = useMemo(() => {
    ...
    return proposals.filter(
      (proposal) => proposal.pin_type === pinTypeFilter
    );
  }, [proposals, pinTypeFilter]);
  ...
  return (
    <AdminMasterDetail
      ...
      maxListWidth={420}
      list={
        <AdminListPane title="PUB">
          <AdminObjectList ariaLabel="PUB stages">
            <AdminObjectListItem
              id="analyze"
              title="Analyze"
              selected={stage === "analyze"}
              onSelect={() => setStage("analyze")}
            />
            ...
            <AdminObjectListItem
              ...
              onSelect={() => setStage("assets")}
            />

            <AdminObjectListItem
              id="package"
              title="Package"
              selected={stage === "package"}
              onSelect={() => setStage("package")}
            />
            ...
            <AdminObjectListItem
              ...
              onSelect={() => setStage("published")}
            />
          </AdminObjectList>
        </AdminListPane>
      }
      detail={
        <AdminDetailPane ariaLabel="PUB">
          ...
          {stage === "analyze" ? (
            <PinterestAnalyzeStage
              ...
              analyzing={analyzing}
              analyzePinterest={analyzePinterest}
              error={error}
              analysis={analysis}
              proposals={proposals}
              ...
              setStage={setStage}
              previewBeforeAfterVideo={previewBeforeAfterVideo}
            />
          ) : (
            <AdminEmptyState
              title={
...
function PinterestAnalyzeStage({
  ...
  return (
   <div className="pub-analyze admin-detail-workarea">
      <div
        style={{
          display: "flex",
          alignItems: "flex-end",
        ...
        <>
          ...
          >
            <div>
              ...
              >
                {analysis.source?.title || "Playlist"}
              </h2>
              ...
              >
                {proposals.length} possible Pinterest asset
            ...
            >
              <label className="admin-field">
                ...
                <select
                  ...
                  value={pinTypeFilter}
                  onChange={(event) =>
                    setPinTypeFilter(event.target.value)
                  }
                >
                  {PIN_TYPES.map((type) => (
                    <option
              ...
              >
                Create Selected Assets ({selectedCount})
              </button>
            </div>
          </div>
          ...
          >
            ...
            >
              <thead>
                <tr>
                  <th style={headerCell}>Use</th>
                  <th style={headerCell}>Order</th>
                  <th style={headerCell}>Type</th>
                  <th style={headerCell}>Before</th>
                  <th style={headerCell}>After / Source</th>
                  <th style={headerCell}>Search Title</th>
              ...
              <tbody>
                {visibleProposals.map((proposal) => {
                    const isPairFormat =
                      proposal.pin_type === "composite"
                      || proposal.pin_type === "before_after_video";

                    const isYoutubeTeaser =
                      proposal.pin_type === "youtube_teaser";

                    const before =
                      isPairFormat || isYoutubeTeaser
                        ? proposal.before
                        : null;
                    ...
                    const source =
                      isPairFormat
                        ? proposal.after
                        : isYoutubeTeaser
                          ? proposal.after_context
                          : proposal.source_item;
                    ...
                    <tr key={proposal.proposal_key}>
                      ...
                      <td style={bodyCell}>
                        ...
                        >
                          ...
                          <option value="before_after_video">
                            Before / After Video
                          </option>
                          ...
                          <option value="idea_palette">
                            Idea + Palette
                          </option>

                          <option value="youtube_teaser">
                            YouTube Teaser
                          </option>
                        </select>
                      </td>
...
function normalizeProposal(proposal) {
  const pinType = proposal.pin_type || "idea";

  const isPairFormat =
    pinType === "composite"
    || pinType === "before_after_video";

  const isYoutubeTeaser =
    pinType === "youtube_teaser";
  ...
  return {
    ...
    include: proposal.include !== false,
...
function SourcePreview({ item }) {
  const imageUrl = browserImageUrl(item?.image_url);
...
function renderImageUrl(value) {
  const raw = browserImageUrl(value);
...
function assetTypeForPinType(pinType) {
  if (pinType === "before_after_video") {
    return "pin_before_after_video";
  }

  if (pinType === "youtube_teaser") {
