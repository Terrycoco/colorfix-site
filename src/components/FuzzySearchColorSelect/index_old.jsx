...
export default function FuzzySearchColorSelect({ onSelect, onFocus, className }) {
  ...
  function runSearch(text) {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        // Minimal normalization: accept {results:[…]} or raw []
        const raw = Array.isArray(data) ? data : (Array.isArray(data?.results) ? data.results : []);
        // Guard against weird responses or error envelopes
        const arr = Array.isArray(raw) ? raw.filter(r => r && (r.name || r.code)) : [];
  ...
  return (
    ...
    >
      <label htmlFor="fuzzy-input" className="dropdown-label">Enter A Color</label>
