...
function Gallery({ items, runQueryById, meta }) {
  ...
  const sections = [];
  const seen = new Map(); // groupName -> section { groupName, items: [] }
  ...
  for (const item of items) {
    ...
    if (!seen.has(label)) {
      const section = { groupName: label || 'TOP', items: [] };
  ...
  if (groupMode === 'lightness') {
    const order = { Light: 1, Medium: 2, Dark: 3 };
    sections.sort((a, b) => {
      const ra = order[a.groupName] ?? 99;
      const rb = order[b.groupName] ?? 99;
      return ra - rb;
    });
  }
